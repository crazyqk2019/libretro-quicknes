...
struct retro_core_option_v2_definition option_defs_chs[] = {
   {
      ...
      "宽高比",
      NULL,
      "此项仅当RetroArch视频设置中的宽高比设为‘由核心提供’时有效。",
      NULL,
      NULL,
      {
         { "PAR", NULL },
...
#ifndef PSP
   {
      ...
      "显示水平过扫描",
      NULL,
      "禁用此项会裁剪掉画面水平方向上两端潜在的随机破碎图像，这些部分在标准CRT电视屏幕上是在画面边框之外的，会被隐藏。",
      NULL,
      NULL,
      {
         { "enabled",  NULL },
   ...
   },
   {
      ...
      "显示垂直过扫描",
      NULL,
      "禁用此项会裁剪掉画面垂直方向上两端潜在的随机破碎图像，这些部分在标准CRT电视屏幕上是在画面边框之外的，会被隐藏。",
      NULL,
      NULL,
      {
         { "disabled", NULL },
   ...
   },
   {
      ...
      "移除角色数限制",
      NULL,
      "移除每条扫描线8个角色数的硬件限制。这会减少闪烁但是可能导致显示问题，因为有些游戏利用此限制来产生特殊效果。",
      NULL,
      NULL,
      {
         { "disabled", NULL },
   ...
   },
   {
      ...
      "音频模式",
      NULL,
      "配置音频模拟。‘非线性’模式模拟NES APU的非线性声道混合；‘线性’模式使用一种精度较低的方法，会降低声音质量但在低端硬件上可以提高性能；‘立体声声像调节’模式通过声像平移技术和混响效果的运用，为NES的单声道输出增添空间深度感。",
      NULL,
      NULL,
      {
         { "nonlinear",      "非线性" },
         { "linear",         "线性" },
         { "stereo panning", "立体声声像调节" },
         { NULL, NULL },
      },
      "nonlinear",
   },
   ...
   },
   {
      ...
      "连发信号宽度（帧数）",
      NULL,
      "当按住连发键A或者B时，指定连发输入脉冲信号的宽度和间隔（以帧数计）。例如，缺省设置值‘3’相当于 (60/(3+3)) = 10赫兹 的连发频率（每秒钟按下10次）。",
      NULL,
      NULL,
      {
         { "1",  NULL },
   ...
   },
   {
      ...
      "允许同时按下相反方向键",
      NULL,
      "启用此项将允许同时按下/快速切换/同时按住左右或者上下键，这可能会导致角色移动问题。",
      NULL,
      NULL,
      {
         { "disabled", NULL },
